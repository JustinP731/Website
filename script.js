...
(() => {
  ...
  const authPhoneInp = document.getElementById("authPhone");
  const authPassInp = document.getElementById("authPass");
  const authCloseBtn = authDialog?.querySelector(".dialog-close");
  const loginToRegister = document.getElementById("registerBtn") || document.getElementById("loginToRegister");
  ...
  if (!getSB()?.auth) {
    console.warn("[Auth] Supabase client missing or not initialized. Expected window.__supabase (or window.supabaseClient) to be a client instance.");
  }
  ...
  // === Avatar refresher (used across pages) ===
  function refreshAvatarFromUser(u = getUser()) {
    const letter = u?.name ? u.name.trim().charAt(0).toUpperCase() : "G";
    if (userStatus) {
      userStatus.textContent = letter;
      userStatus.title = u?.name || "Guest";
      userStatus.setAttribute("aria-label", `User: ${u?.name || "Guest"}`);
    }
    document.querySelectorAll(".profile-circle, .page-profile .avatar-lg").forEach(el => { el.textContent = letter; });
  }


  let __authHydrating = false;

  // wait until the Supabase client is actually on window, then attach
  (function attachAuthListener() {
    const sb = getSB();
    if (!sb?.auth?.onAuthStateChange) {
      setTimeout(attachAuthListener, 100);
      return;
    }
  ...
  async function waitForSession(timeoutMs = 4000, stepMs = 120) {
    const t0 = Date.now();
  ...
  // Helpers for login button state + clearing inline errors
...
const closeBtn = panel?.querySelector(".room-close");
  ...
  menuLogin?.addEventListener("click", () => {
    toggleMenu(false);
    if (authPhoneInp) authPhoneInp.value = "";
    if (authEmailInp) authEmailInp.value = "";  // harmless if present
    if (authPassInp) authPassInp.value = "";
    authDialog?.showModal();
    (authPhoneInp || authEmailInp)?.focus();
  });
  ...
  function validateLoginField(input) {
    ...
    if (input === authPhoneInp) {
      ok = isPhoneVal(v); msg = "Please enter a valid phone number.";
    } else if (input === authEmailInp) {
      ok = /^\S+@\S+\.\S+$/.test(v); msg = "Please enter a valid email.";
    } else if (input === authPassInp) {
      ok = v.length > 0; msg = "Please enter your password.";
    }
  ...
  function validateLoginForm() {
    const fields = [authPhoneInp || authEmailInp, authPassInp].filter(Boolean);
    let firstBad = null;
  ...
  [authPhoneInp || authEmailInp, authPassInp].forEach(el => {
  ...
  authForm?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const raw = (authPhoneInp?.value || authEmailInp?.value || "").trim();
      ...
      if (!sb?.auth?.signInWithPassword) {
        setFieldError(authPassInp, "Auth service unavailable on this page.");
        return;
      }
      ...
      if (error) {
        setFieldError(authPassInp, error.message || "Login failed");
        return;
      }

      // Optionally wait until the session is actually on disk
      try {
        const t0 = Date.now();
      ...
      // Keep your profile row in sync (safe if it exists already)
      try { await ensureProfileRow?.(); } catch { }

      // Pull profile and cache locally so the avatar shows immediately
      try {
        ...
        if (user?.id) {
          ...
          if (row) {
            // prefer full_name; fall back to something sensible
      ...
      console.error(err);
      setFieldError(authPassInp, "Unexpected error logging in.");
    } finally {
  ...
  regSend?.addEventListener("click", () => {
    const raw = (regPhone?.value || "").trim();
    if (!isPhoneVal(raw)) {
      setFieldError(regPhone, "Enter a valid phone number before sending a code.");
      regPhone?.focus();
      return;
    }
    clearFieldError(regPhone);
    startOtpCountdown(60); // TODO: call your backend / Supabase OTP here
  });


  // — Register: validators + live + submit
  // field-level validation for the Register dialog
  ...
  [regName, regPhone, regCode, regPass, regPass2].forEach(el => {
  ...
  function validateRegisterForm() {
    const fields = [regName, regPhone, regCode, regPass, regPass2].filter(Boolean);
    let firstBad = null;
    const okAll = fields.every(f => {
  ...
  regForm?.addEventListener("submit", (e) => {
    ...
    const name = (regName?.value || "User").trim();
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      const { data: { session } = { session: null } } =
        sb0?.auth?.getSession ? await sb0.auth.getSession() : { data: { session: null } };
      if (session) {
        await hydrateUserFromProfile?.();
        await updateAuthUI?.();
      }
  ...
  function closePanel() {
    if (!panel) { exitSplitLayout(); return; }
...
const homeIsActive = !!document.getElementById('homeBtn')?.classList?.contains('active');
toggleHomeOnly(homeIsActive);
