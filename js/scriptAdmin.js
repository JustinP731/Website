...
(() => {
  ...
  const sb =
    (window.auth && typeof auth.getClient === "function" && auth.getClient()) ||
    window.__supabase ||
    window.supabaseClient;

  if (!sb) {
    console.error("Supabase client missing — make sure auth.js loaded before scriptAdmin.js");
  }
  ...
  // ========== rebound if not admin/staff =======================

  async function waitForInitialSession(sb, timeoutMs = 3000) {
    ...
    return new Promise((resolve) => {
      ...
      const { data: { subscription } } = sb.auth.onAuthStateChange((evt, sess) => {
  ...
  async function requireAdminOrStaff(opts = {}) {
    ...
    // If auth.js is present, delegate to it (single source of truth)
    if (window.auth && typeof window.auth.requireAdminOrStaff === "function") {
      return window.auth.requireAdminOrStaff({ homePath });
    }

    // Fallback inline check (if auth.js didn't expose the helper for any reason)
    if (!sb) return location.replace(homePath);

    // 1) Ensure we have a session
    const { data: sessionData, error: sessErr } = await sb.auth.getSession();
    const session = sessionData && sessionData.session;
    if (sessErr || !session) return location.replace(homePath);

    // 2) Look up role (prefer profiles.role; fallback to app_metadata.role)
    let role = null;
    try {
      const { data: prof } = await sb
        ...
        .maybeSingle();
      if (prof && prof.role) role = String(prof.role).toLowerCase();
    } catch (_) {
      /* noop */
    }
    if (!role) {
      const metaRole = session.user?.app_metadata?.role;
      if (metaRole) role = String(metaRole).toLowerCase();
    }

    // 3) Bounce if not admin/staff
    if (!(role === "admin" || role === "staff")) return location.replace(homePath);
  }

  // --- bounce to Home when user signs out (safety net)
  // Safety net: if user signs out anywhere, bounce to Home
  function installAdminSignOutBounce(opts = {}) {
    const homePath = opts.homePath || "index.html";
    const sb =
      (window.auth && typeof auth.getClient === "function" && auth.getClient()) ||
      window.__supabase || window.supabaseClient;
    if (!sb) return;
    sb.auth.onAuthStateChange((evt) => {
      if (evt === "SIGNED_OUT") location.replace(homePath);
    });
  }

  // Wire the logout button to fully sign out the account, then go Home
  function wireSignOutButton(selector = "#adminSignOut") {
    const sb =
      (window.auth && typeof auth.getClient === "function" && auth.getClient()) ||
      window.__supabase || window.supabaseClient;
    ...
    btn.addEventListener("click", async () => {
      try {
        await sb.auth.signOut();           // clears Supabase session in the browser
        location.replace("index.html");    // hard redirect just in case
      } catch (e) {
  ...
  // --- simple dropdown toggle (click outside + Esc closes)
  function wireAdminMenuToggle(btnSel = "#adminMenuBtn", menuSel = "#adminMenu") {
  ...
  // Use default image for guests; for logged-in users: avatar if present, else letter
  async function hydrateAdminUserStatus() {
    const badge = document.getElementById("adminUserStatus");
    if (!badge) return;

    const DEFAULT_AVATAR = "pngs/default-avatar.png"; // put your default person icon here
    const { data: { session } } = await sb.auth.getSession();
    const u = session?.user;

    // Guest: always show default person image
    if (!u) {
      badge.innerHTML = `<img src="${DEFAULT_AVATAR}" alt="User avatar" class="h-9 w-9 rounded-full object-cover" />`;
      badge.title = "User: Guest";
      badge.setAttribute("aria-label", "User: Guest");
      return;
    }

    // Logged-in: try to resolve display name and avatar URL
    let name =
    ...
    try {
      ...
      if (prof?.name) name = String(prof.name).trim();
      if (prof?.avatar_url) avatarUrl = prof.avatar_url || avatarUrl;
    } catch { /* ignore */ }

    if (avatarUrl) {
      // Logged-in with photo
      badge.innerHTML = `<img src="${avatarUrl}" alt="User avatar" class="h-9 w-9 rounded-full object-cover" />`;
    } else {
      // Logged-in, no photo -> first letter
  ...
  // Let the profile circle toggle the same dropdown as the hamburger
  function wireProfileCircleTogglesMenu(circleSel = "#adminUserStatus", menuBtnSel = "#adminMenuBtn") {
    const circle = document.querySelector(circleSel);
    const menuBtn = document.querySelector(menuBtnSel);
    if (!circle || !menuBtn) return;
    circle.addEventListener("click", () => menuBtn.click());
  }
...
Cazza Zamora Residences`;
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('gate-splash')?.remove();
    installAdminSignOutBounce({ homePath: "index.html" });
    wireSignOutButton("#adminSignOut");
    wireAdminMenuToggle("#adminMenuBtn", "#adminMenu");
    await hydrateAdminUserStatus();
    sb?.auth.onAuthStateChange(async () => { await hydrateAdminUserStatus(); });
    wireProfileCircleTogglesMenu("#adminUserStatus", "#adminMenuBtn");
