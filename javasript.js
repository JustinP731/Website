...
// ✅ Save Room — validate + append to #room-list
...
if (saveBtn) {
  ...
  // clear red outline while typing
  [typeInput, priceInput].forEach(el => el?.addEventListener('input', () => clearInvalid(el)));
  ...
  saveBtn.addEventListener('click', () => {
    ...
    clearInvalid(typeInput);
    clearInvalid(priceInput);

    // ✋ validations
    if (!type) {            // missing room type
      markInvalid(typeInput);
      typeInput?.focus();
      return;
    }
    if (!priceRaw || isNaN(price) || price <= 0) {  // missing/invalid price
